...
for ii = 2:length(catalog.data)
       if(abs(catalog.data(ii,1)-catalog.data(ii-1,1)) <= secondsMax/24/60/60)
           if(distance(catalog.data(ii,2:3),catalog.data(ii-1,2:3)) <= kmMax/111.12)
              ...
              disp([datestr(catalog.data(ii-1,1),'yyyy-mm-dd HH:MM:SS.FFF'),'  ',catalog.id{ii-1},' ',num2str(catalog.data(ii-1,2:5))]);
              disp([datestr(catalog.data(ii,1),'yyyy-mm-dd HH:MM:SS.FFF'),'  ',catalog.id{ii},' ',num2str(catalog.data(ii,2:5))]);
              disp('-----------------------')
              dup = dup+1;
           end
       end
